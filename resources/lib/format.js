...
Ext.apply(Ext.util.Format, {
  ...
  formatOffersStep: function(val) {
    return Ext.util.Format.formatPrice(val, {decimalPrecision: 2});
  },
  bool: function(val) {
    return val?'Да':'Нет';
  },
  boolLower: function(val) {
    return val ? 'да' : 'нет';
  },
  boolSetUnsetLower: function (val) {
    return val ? 'установлено' : 'не установлено';
  },
  ...
  viewPhone: function(phone) {
    var sep = phone.indexOf(' ');
    var piece_phone = (sep == -1) ? phone : phone.slice(0,sep);
    var piece_extra = (sep == -1) ? null : Ext.util.Format.trim(phone.slice(sep));
    return piece_extra ? piece_phone +' (доб. '+piece_extra +')': piece_phone;
  }
});
